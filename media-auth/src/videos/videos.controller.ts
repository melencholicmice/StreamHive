...
export class VideosController {
  ...
  async startUpload(
    ...
    try {
      const uploadId = await this.videoService.initiateMultipartUpload(
        body.key,
        user.id,
        body.videoName,
      );
  ...
  async getPresignedUrls(
    ...
    const url = await this.videoService.generatePresignedUrlByPartNumber(
      key,
      uploadId,
      partNumber,
      user.id,
  ...
  async completeUpload(
    ...
    await this.videoService.completeMultipartUpload(
      body.key,
      body.uploadId,
      body.parts,
      user.id,
    ...
    await this.videoService.updateBucketLocation(
      user.id,
      body.videoName,
      body.videoId,
      body.key,
    ...
    console.log(
      'key:= ' +
        `${this.videoService.createKeyDirectory(user.id, body.videoName)}${body.key}`,
    );
  ...
  @Get('user-videos')
  @UseGuards(JwtAuthGuard)
  async getAllUserVideos(@CurrentUser() user: User) {
